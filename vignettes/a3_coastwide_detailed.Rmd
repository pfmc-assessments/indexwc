---
title: "Coastwide indices: detailed example"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Coastwide indices: detailed example}
...
```{r fit-simple, eval=FALSE, echo=TRUE}
family_obj <- eval(rlang::parse_expr(configuration_to_run$family))
...
```{r diagnose, eval=FALSE, echo=TRUE}
...
```{r sanity, eval=FALSE, echo=TRUE}
diagnostics$sanity
```

The model is returned with 
```{r diagmodel, eval=FALSE, echo=TRUE}
diagnostics$model
```

The formula is returned with 
```{r diagformula, eval=FALSE, echo=TRUE}
diagnostics$formula
```

The log-likelihood and AIC are returned with 
```{r diagaic, eval=FALSE, echo=TRUE}
diagnostics$loglike
diagnostics$aic
```
...
```{r diageffects, eval=FALSE, echo=TRUE}
diagnostics$effects
```

The SPDE mesh plot can be accessed with
```{r diagmesh, eval=FALSE, echo=TRUE}
...
```{r diagdensity, eval=FALSE, echo=TRUE}
...
```{r diagresidmaps, eval=FALSE, echo=TRUE}
...
```{r diagdate, eval=FALSE, echo=TRUE}
diagnostics$date
```

and the session info with 

```{r diagsession, eval=FALSE}
...
```{r diagareas, eval=FALSE, echo=TRUE}
available_areas()
```
...
```{r diagindex, eval=FALSE, echo=TRUE}
...
```{r save-output, echo = FALSE, eval = FALSE}
save_index_outputs(
  fit = fit_simple,
  diagnostics = diagnostics,
  indices = index,
  dir = paste0(getwd(), "/yellowtail_example")
)
```
