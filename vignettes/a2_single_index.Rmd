...
First we'll load in the configuration file. This step is not needed as the formulas, etc. may be specified from scratch, but this file is used to keep models reproducible across assessment cycles. The configuration file brings in specific settings by survey and species that are used to subset the data and define the parameterization of the index estimation.  This file is included as both an `rda` file that is loaded with the package but is also available for users who download the source code in the `data-raw` folder in the package repository. 
...
There are two unique index formulas available for yellowtail rockfish:

```{r printform, eval=TRUE, echo=TRUE}
configuration_ytk_wcgbt[, "formula"]
```

We are going to use the default configuration.  Please see the vignette on multiple area indices for an example run with the other formulation (with a split at Cape Mendocino) available for yellowtail rockfish. 
...
This code block pulls in a saved data file from the NWFSC data warehouse and applies filters (based on latitude, depth, and year) to each observation.

Alternatively, the user can pull the data directly with the nwfscSurvey R package.
...
A key point to highlight is that in this code, we use `sdmTMBcontrol()` to pass in a list that contains the variables above we use to "map off" (i.e., not estimate) and initialize the parameters. For datasets without missing values, you may not want to include this `sdmTMBcontrol()` line, but it may be useful to adjust the `newton_loops`, etc.
...
All diagnostics and plots are generated by the `diagnose()` function. This function takes a fitted object and prediction grid as input, and optionally a directory name (`dir`) for saving results.
...
Density plots are made separately for each year, and stored 2 per page in nested lists. They can be accessed with

```{r diagdensity, eval=TRUE, echo=TRUE}
diagnostics$density_plots[[10]]
```

Maps are also made of the residuals. Accessing these is not as straightforward, but is done with pagination
...
and the session information with 
...
```{r diagprintcogs, eval=TRUE, echo=TRUE}
index$cogs
```
Note: In the simple model example, spatial and spatiotemporal fields are "off" and COG estimates do not vary by year.
